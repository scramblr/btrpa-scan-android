...
object ExportManager {
    ...
    private fun writeCsv(writer: FileWriter, devices: List<BleDeviceInfo>) {
        // Header
        writer.write("timestamp,address,address_type,name,rssi,avg_rssi,tx_power,est_distance,manufacturer_data,service_uuids,irk_resolved\n")
        
        devices.forEach { device ->
            val row = listOf(
                device.timestamp,
                device.address,
                if (device.isRpa) "RPA" else "Public",
    ...
    private fun writeJson(writer: FileWriter, devices: List<BleDeviceInfo>) {
        ...
        val records = devices.map { device ->
            mapOf(
                ...
                "address_type" to if (device.isRpa) "RPA" else "Public",
                ...
                "irk_resolved" to device.irkResolved
            )
        }
