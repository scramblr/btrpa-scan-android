...
class MainActivity : AppCompatActivity(), BleScanner.ScanListener {
    ...
    private lateinit var textRpaCount: TextView
    ...
    private fun setupViews() {
        ...
        textRpaCount = findViewById(R.id.textRpaCount)
    ...
    private fun setupModeChips() {
        ...
        chipDiscover.setOnClickListener {
            ...
            // Don't reset timeout here - let user's settings persist
        }
    ...
    private fun showIrkDialog() {
        ...
        val helpButton = MaterialButton(this, null, com.google.android.material.R.attr.materialButtonOutlinedStyle).apply {
            text = "What is an IRK? How do I get one?"
            setOnClickListener { showIrkHelpDialog() }
        }
        container.addView(helpButton)
        
        AlertDialog.Builder(this)
            .setTitle("Identity Resolving Key (IRK)")
            .setMessage("Enter the 16-byte IRK in hex format to resolve RPAs:")
            .setView(container)
            .setPositiveButton("OK") { _, _ ->
    ...
    private fun showIrkHelpDialog() {
        val scrollView = ScrollView(this)
        val textView = TextView(this).apply {
            text = IrkResolver.getIrkHelpText()
            setPadding(48, 32, 48, 32)
            textSize = 14f
        }
    ...
    private fun observeScanner() {
        ...
        lifecycleScope.launch {
            scanner.stats.collectLatest { stats ->
                ...
                textRpaCount.text = "RPAs: ${stats.rpaCount}"
    ...
    private fun showDeviceDetails(device: BleDeviceInfo) {
        val message = buildString {
            ...
            appendLine("Address Type: ${device.addressTypeLabel}")
            ...
            if (device.isRpa) {
                appendLine("\nThis device uses a Resolvable Private Address (RPA).")
                device.irkResolved?.let { resolved ->
                    if (resolved) {
                        appendLine("✓ Successfully resolved with provided IRK")
                    } else {
                        appendLine("✗ Does not match provided IRK")
                    }
                }
            }
        }
    ...
    private fun showSettingsDialog() {
        ...
        val switchContinuous = dialogView.findViewById<SwitchMaterial>(R.id.switchContinuousScanning)
        val layoutTimeout = dialogView.findViewById<com.google.android.material.textfield.TextInputLayout>(R.id.layoutTimeout)
        ...
        val isContinuous = (timeout == null)
        switchContinuous.isChecked = isContinuous
        layoutTimeout.visibility = if (isContinuous) View.GONE else View.VISIBLE
        ...
        if (timeout != null) {
            editTimeout.setText((timeout!! / 1000).toString())
        } else {
            editTimeout.setText("30")
        }
        
        // Toggle timeout field visibility based on continuous switch
        switchContinuous.setOnCheckedChangeListener { _, isChecked ->
            layoutTimeout.visibility = if (isChecked) View.GONE else View.VISIBLE
        }
        ...
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("Apply") { _, _ ->
                ...
                // Handle continuous vs timed scanning
                timeout = if (switchContinuous.isChecked) {
                    null  // null = infinite/continuous scanning
                } else {
                    editTimeout.text.toString().toLongOrNull()?.times(1000) ?: 30_000L
                }
                ...
                // Show confirmation
                val modeText = if (timeout == null) "continuous" else "${timeout!! / 1000} seconds"
                Toast.makeText(this, "Scan timeout: $modeText", Toast.LENGTH_SHORT).show()
            }
            .setNegativeButton("Cancel", null)
            .show()
    }
